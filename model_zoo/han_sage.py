...
class HANSage(nn.Module):
    def __init__(self, adj_lists, feat_data, num_classes, embed_dim, num_sample, num_layers, is_cuda=True):
        ...
        for i, adj_list in enumerate(adj_lists):
            ...
            print(device)
            self.encoders.append(GraphSage(adj_list, feat_data, num_classes, self.embed_dim, num_sample, num_layers, self.is_cuda, as_view=True, cuda_device=device).to(device))
    ...
    def get_embedding(self, nodes):
        hiddens = [re_view(encoder.get_embedding(nodes).to("cuda:0")) for encoder in self.encoders] # 最后放在cuda:0计算loss
...
class HANSageHander(BaseHander):
    ...
    def build_model(self):
        ...
        self.loss_func = nn.CrossEntropyLoss(weight=torch.Tensor([1, 4]).cuda())
    ...
    def custom_init(self, freeze=False):
        ...
        all_views = ['app_author_app', 'app_component_app', 'app_drebin_app']
